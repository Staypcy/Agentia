...
### ✨检索增强生成 (RAG) 与长期记忆（后续会迁移到LangChain等）
如果每次决策都从零开始，LLM 就像一个没有经验的“白板”。我们实现了 **轻量级 RAG**：
- 每次决策的完整上下文（状态、环境、动作）存入 **ChromaDB** 向量数据库。
- 下一次决策前，用当前状态检索最相似的 **3 个历史案例**，将其作为参考注入 Prompt。
- LLM 可以“回忆起”过去在类似情况下做了什么，从而使行为更连贯，避免重复踩坑。

这是一个经典的 **“经验回放”** 式记忆的现代实现，也是面试中常被问到的 RAG 落地场景。

### ✨为什么没有使用Tool Calling？
 **(ps:其实是某人还没开始学LangChain...后续会改进的啦>_<)**
- **Tool Calling (函数调用)**：当前 LLM 输出固定词表中的动作，由 C++ 端解析并执行，本质上是一种“受控的自然语言接口”。如果未来想让 LLM 自主决定调用不同的外部服务（如地图查询、资源计算），可将动作映射升级为 Tool Calling，这会让系统更规范和可扩展。
...
| **智能体长期记忆缺失** | RAG,使用 ChromaDB 存储 (状态, 动作) 对，采用 Sentence-Transformers 生成语义向量；决策时检索 top-3 相似状态，注入 LLM Prompt 作为 RAG 参考，提升决策连续性和稳定性。 |
